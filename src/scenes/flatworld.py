...
from src.core.types import Vec3, Point2d, Rect2d
    ...
    def generate_flat_world(
        ...
        if obstacles_mode == "random":
            ...
            for i in range(num_obstacles):
                ...
                x_pos: float = random.uniform(
                    -arena_size_x + sx * 2, arena_size_x - sx * 2
                )
                y_pos: float = random.uniform(
                    -arena_size_y + sy * 2, arena_size_y - sy * 2
                )

                # Skip if too close to spawn (0, 0) or goal
                dist_to_spawn: float = (x_pos**2 + y_pos**2) ** 0.5
                dist_to_goal: float = (
                    (x_pos - goal_position.x) ** 2 + (y_pos - goal_position.y) ** 2
                ) ** 0.5
